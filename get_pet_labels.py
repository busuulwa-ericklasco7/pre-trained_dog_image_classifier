...
def get_pet_labels(image_dir):
    ...
    """
    #creates list of files in dictionary
...
# Creates empty dictionary for the results (pet labels, etc.)
    ...
    for idx in range(0, len(in_files), 1):
        if in_files[idx][0] != ".":
            ...
            pet_label = ""
            #This start of the code have added where pass was
            filenames = [file.lower for file in in_files]
            for file in filenames:
                pet_names = str(file).split("_")
                for pet in pet_names:
                    if pet.isalpha():
                        pet_label += pet + " "
            pet_label = [pet.strip() for pet in pet_label]
    
            #This end of the code i have added
